...
/// Summary returned by `SrdInstance::plan_translucent_shadow_passes`.
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub struct SrdTranslucentShadowPlan {
    pub final_output_scratch_index: u16,
    pub dispatch_count: usize,
}

/// External graph images consumed by the Translucent Shadow Stabilizer dispatches.
pub struct SrdTranslucentShadowStabilizerInputs<'a> {
    pub translucency: &'a GraphImage,
...
/// Compute shader programs used by the Translucent Shadow Stabilizer executor.
pub struct SrdTranslucentShadowStabilizerPrograms {
...
impl SrdTranslucentShadowStabilizerPrograms {
    ...
    pub fn load(engine: &Engine) -> Result<Self> {
        Ok(Self {
            ...
            accumulate: ComputeProgram::load(
                engine,
                Self::srd_engine_shader("srd_translucency_accumulate.slang"),
            )?,
            filter: ComputeProgram::load(
                engine,
                Self::srd_engine_shader("srd_translucency_filter.slang"),
            )?,
        })
    }
    ...
    pub fn program_for_shader_label(&self, shader_label: &str) -> Option<&ComputeProgram> {
        match shader_label {
            ...
            "srd_translucency_accumulate" => Some(&self.accumulate),
            "srd_translucency_filter" => Some(&self.filter),
            _ => None,
        }
    }
}

/// Renderer-side executor for SRD Translucent Shadow Stabilizer compute dispatches.
pub struct SrdTranslucentShadowStabilizerExecutor<'a> {
    programs: &'a SrdTranslucentShadowStabilizerPrograms,
}

impl<'a> SrdTranslucentShadowStabilizerExecutor<'a> {
    pub const fn new(programs: &'a SrdTranslucentShadowStabilizerPrograms) -> Self {
        Self { programs }
    }
    ...
    pub fn execute_plan(
        ...
        inputs: SrdTranslucentShadowStabilizerInputs<'_>,
        plan: SrdTranslucentShadowPlan,
        output_name: &str,
    ) -> Result<GraphImage> {
        ...
        let output = scratch
            ...
            .ok_or_else(|| {
                crate::Error::InvalidInput(format!(
                    "SRD translucent shadow final output references missing scratch index {}",
    ...
    fn execute_dispatch(
        ...
        let pipeline = instance
            ...
            .ok_or_else(|| {
                crate::Error::InvalidInput(format!(
                    "SRD translucent shadow dispatch '{}' references missing pipeline {}",
        ...
        let program = self
            ...
            .ok_or_else(|| {
                crate::Error::InvalidInput(format!(
                    "SRD translucent shadow dispatch '{}' references unsupported shader label '{}'",
                    dispatch.name, pipeline.shader_label
                ))
            })?;
        let storage_resource = single_storage_resource(dispatch)?;
        let target = resolve_translucent_shadow_resource(storage_resource, inputs, history, scratch)?;
        let target_binding =
        ...
        for resource in &dispatch.resources {
            match resource.descriptor_type {
                SrdDescriptorType::Texture => {
                    ...
                    let image = resolve_translucent_shadow_resource(resource, inputs, history, scratch)?;
                ...
                SrdDescriptorType::Sampler | SrdDescriptorType::UniformBuffer => {
                    return Err(crate::Error::InvalidInput(format!(
                        "SRD translucent shadow dispatch '{}' contains unsupported descriptor type {:?}",
        ...
        if pipeline.has_constants || dispatch.constants_size > 0 {
            let range = dispatch.constants_range.ok_or_else(|| {
                crate::Error::InvalidInput(format!(
                    "SRD translucent shadow dispatch '{}' is missing constants",
                    dispatch.name
                ))
            })?;
...
fn resolve_translucent_shadow_resource<'a>(
    resource: &SrdResourceDesc,
    inputs: &'a SrdTranslucentShadowStabilizerInputs<'a>,
    history: &'a [GraphImage],
    scratch: &'a [GraphImage],
) -> Result<&'a GraphImage> {
    match (resource.slot, resource.pool_index) {
        (SrdResourceSlot::TranslucencyInput, None) => Ok(inputs.translucency),
        ...
        (slot, pool_index) => Err(crate::Error::InvalidInput(format!(
            "SRD translucent shadow executor cannot resolve resource {slot:?} pool {pool_index:?}"
        ))),
    }
}
...
fn missing_pool_resource(resource: &SrdResourceDesc) -> crate::Error {
    crate::Error::InvalidInput(format!(
        "SRD translucent shadow executor missing {:?} pool resource {:?}",
        resource.slot, resource.pool_index
    ))
}

impl SrdInstance {
    pub fn register_translucency_accumulate_pipeline(&mut self) -> Result<usize> {
        self.register_pipeline(SrdPipelineDesc {
            name: "SRD Translucency Accumulate".into(),
            debug_label: "SRD Translucency Accumulate".into(),
            shader_label: "srd_translucency_accumulate".into(),
            has_constants: true,
            workgroup_size: [8, 8, 1],
        })
    }

    pub fn register_translucency_filter_pipeline(&mut self) -> Result<usize> {
        self.register_pipeline(SrdPipelineDesc {
            name: "SRD Translucency Filter".into(),
            debug_label: "SRD Translucency Filter".into(),
            shader_label: "srd_translucency_filter".into(),
            has_constants: true,
            workgroup_size: [8, 8, 1],
        })
    }

    /// Prepare all translucent-shadow stabilizer resources and pipelines.
    pub fn prepare_translucent_shadow(
        &mut self,
        denoiser_id: SrdDenoiserId,
    ) -> Result<SrdTranslucentShadowResources> {
        if self.mode_for_id(denoiser_id) != Some(SrdDenoiserMode::TranslucentShadowStabilizer) {
            return Err(Error::InvalidInput(format!(
                "SRD denoiser id {} is not a TranslucentShadowStabilizer denoiser",
                denoiser_id.get()
            )));
        }
        ...
        let label = format!("translucency_history_{id}");
        let write_index = self.add_history_texture(SrdTextureDesc {
            name: format!("{label}_write"),
            debug_label: format!("SRD Translucency History {id} Write"),
        ...
        let read_index = self.add_history_texture(SrdTextureDesc {
            ...
            debug_label: format!("SRD Translucency History {id} Read"),
            slot: SrdResourceSlot::HistoryPool,
            format: Format::Rgba16Float,
            pool: Some(SrdPoolClass::History),
            downsample_factor: 1,
        ...
        let accumulate_pipeline = self.register_translucency_accumulate_pipeline()?;

        let filter_scratch_index = self.add_unique_scratch_texture(SrdTextureDesc {
            name: format!("translucency_filter_{id}"),
            debug_label: format!("SRD Translucency Filter {id}"),
            slot: SrdResourceSlot::ShadowTranslucencyOutput,
            format: Format::Rgba16Float,
            pool: Some(SrdPoolClass::Scratch),
            downsample_factor: 1,
        })?;
        let filter_pipeline = self.register_translucency_filter_pipeline()?;

        Ok(SrdTranslucentShadowResources {
            clear_pipeline,
            surface_mask,
            reproject,
            accumulate: SrdTranslucentShadowAccumulateResources {
                history_ring_index: accumulate_ring_index,
                pipeline: accumulate_pipeline,
            },
            filter: SrdTranslucentShadowFilterResources {
    ...
    /// Plan one frame of translucent-shadow denoising.
    ///
    /// Emitted order: optional clear, surface mask, reproject, accumulate, filter.
    pub fn plan_translucent_shadow_passes(
        &mut self,
        denoiser_id: SrdDenoiserId,
        resources: SrdTranslucentShadowResources,
        rect_size: UVec2,
    ) -> Result<SrdTranslucentShadowPlan> {
        if self.mode_for_id(denoiser_id) != Some(SrdDenoiserMode::TranslucentShadowStabilizer) {
            return Err(Error::InvalidInput(format!(
                "SRD denoiser id {} is not a TranslucentShadowStabilizer denoiser",
                denoiser_id.get()
        ...
        let translucency_settings = match self.denoiser_settings(denoiser_id) {
            Some(SrdFamilySettings::TranslucentShadow(s)) => *s,
            _ => super::settings::SrdTranslucentShadowSettings::default(),
        };

        // Translucency Accumulate: translucency + reproject + surface mask + history_read → history_write
        {
            let constants = SrdTranslucentShadowAccumulateConstants {
                rect_extent: [rect_size.x.max(1), rect_size.y.max(1)],
                mask_extent: [mask_extent.x, mask_extent.y],
                history_frame_budget: translucency_settings.history_frame_budget.max(1) as f32,
                use_surface_mask: 1,
                tile_size: tile,
                _pad: 0,
            };
            ...
            let dispatch = SrdPassBuilder::new(
                "SRD Translucency Accumulate",
                denoiser_id,
                resources.accumulate.pipeline,
            )
            .read(SrdResourceSlot::TranslucencyInput)
        ...
        // Translucency Filter: accumulated history + depth + normal + surface mask → filtered output
        {
            let constants = SrdTranslucentShadowFilterConstants {
                rect_extent: [rect_size.x.max(1), rect_size.y.max(1)],
                mask_extent: [mask_extent.x, mask_extent.y],
                normal_weight_power: translucency_settings.normal_weight_power,
                spatial_radius: translucency_settings.spatial_radius,
                use_surface_mask: 1,
                tile_size: tile,
            };
            let constants_range = self.push_typed_constants(&constants);
            let dispatch = SrdPassBuilder::new(
                "SRD Translucency Filter",
                denoiser_id,
