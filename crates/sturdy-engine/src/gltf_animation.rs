...
pub fn load_skins(doc: &gltf::Document, buffers: &[gltf::buffer::Data]) -> Vec<GltfSkin> {
    ...
    doc.skins()
        .map(|skin| {
            ...
            let joint_to_skin_idx: std::collections::HashMap<usize, usize> = joints
                .iter()
                .enumerate()
...
pub fn load_animations(doc: &gltf::Document, buffers: &[gltf::buffer::Data]) -> Vec<AnimationClip> {
    doc.animations()
        .map(|anim| {
            ...
            for ch in anim.channels() {
                ...
                let times: Vec<f32> = match reader.read_inputs() {
                    Some(t) => {
                        let v: Vec<f32> = t.collect();
                        duration = duration.max(v.last().copied().unwrap_or(0.0));
                        v
                    }
                    None => continue,
                };
                ...
                let values: Vec<f32> = match reader.read_outputs() {
                    ...
                    Some(gltf::animation::util::ReadOutputs::Rotations(it)) => it
                        .into_f32()
                        .flat_map(|[x, y, z, w]| [x, y, z, w])
                        .collect(),
...
pub fn load_skinned_vertices(
    ...
    let joints: Vec<[f32; 4]> = match reader.read_joints(0)? {
        gltf::mesh::util::ReadJoints::U8(it) => it
            ...
            .collect(),
        gltf::mesh::util::ReadJoints::U16(it) => it
            .map(|[a, b, c, d]| [a as f32, b as f32, c as f32, d as f32])
            .collect(),
    };
    ...
    if reader.read_tangents().is_none() {
        ...
        let mut v3d: Vec<crate::Vertex3d> = vertices
            .iter()
            .map(|v| crate::Vertex3d {
                position: v.position,
                normal: v.normal,
                uv: v.uv,
