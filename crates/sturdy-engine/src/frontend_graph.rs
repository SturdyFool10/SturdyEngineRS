...
// ── ShaderProgram, ShaderName, ShaderProgramDesc, SlangEntryPoints ────────────
// These types live in shader_program.rs and are re-exported above via
...
fn submit_pending_passes(inner: &mut RenderFrameInner) -> Result<()> {
    ...
    // Phase 2: schedule.
    let order = scheduler::schedule_pass_order(&resolved, &inner.ordering_constraints);
