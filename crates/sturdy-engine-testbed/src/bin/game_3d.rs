...
use sturdy_engine::{
    DeferredPass, Engine, FixedUpdateContext, GameApp, GameConfig, GameContext,
    InputHub, Keybind, KeyModifier, Mesh, MeshProgram, ObjectKind, OrbitCamera,
    PointLight, RectLight, Scene, ShaderProgram, ShellFrame, SkyConfig, Surface,
    SurfaceImage, MaterialExpr, UnifiedMaterial, WindowConfig,
};
...
impl GameApp for Game3d {
    ...
    fn init(engine: &Engine, _surface: &Surface) -> sturdy_engine::Result<Self> {
        ...
        scene.set_unified_material(g, UnifiedMaterial::pbr_metallic_roughness("ground")
        ...
        scene.set_unified_material(p, UnifiedMaterial::pbr_metallic_roughness("pedestal")
            ...
            .build());
        scene.add_object_at(p,
            Mat4::from_scale_rotation_translation(Vec3::new(1.2, 0.3, 1.2), Quat::IDENTITY, Vec3::new(0.0, 0.15, 0.0)),
            ObjectKind::Static);

        // Sphere on pedestal — procedural pulsing emissive.
        let s = scene.add_mesh(Mesh::uv_sphere(engine, 0.45, 32, 32)?, MeshProgram::lit(engine)?);
        scene.set_unified_material(s, UnifiedMaterial::pbr_metallic_roughness("sphere")
        ...
        deferred.sky = SkyConfig { turbidity: 3.0, exposure: 1.2, ..Default::default() };
        let passthrough = ShaderProgram::passthrough(engine)?;
        let camera      = OrbitCamera::new(Vec3::new(0.0, 0.8, 0.0), 8.0);

        Ok(Self { engine: engine.clone(), scene, deferred, passthrough, camera, time: 0.0, orbit_n })
    }
    ...
    fn fixed_update(&mut self, ctx: &FixedUpdateContext<'_>) -> sturdy_engine::Result<()> {
        ...
        let spd = 1.2f32;
        let zsz = 3.0f32;
        if a.is_held("left")  { self.camera.yaw   -= spd * dt; }
        if a.is_held("right") { self.camera.yaw   += spd * dt; }
        if a.is_held("up")    { self.camera.pitch  = (self.camera.pitch + spd * 0.5 * dt).min(1.5); }
        if a.is_held("down")  { self.camera.pitch  = (self.camera.pitch - spd * 0.5 * dt).max(-0.1); }
        if a.is_held("zin")   { self.camera.distance = (self.camera.distance - zsz * dt).max(2.0); }
        if a.is_held("zout")  { self.camera.distance = (self.camera.distance + zsz * dt).min(22.0); }
    ...
    fn render(
        ...
        let t = self.time + ctx.fixed_alpha * ctx.fixed_step.map(|s| s.as_secs_f32()).unwrap_or(0.0);
