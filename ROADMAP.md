...
- [x] Add a Rust-facing shader API where the app can write something equivalent to:

  ```rust
  let shader = engine.load_slang_source(
  ...
  (The `.shaders()` accessor and shader cache are deferred until the async/hot-reload architecture is designed.)
...
- [ ] Use reflection to derive or validate descriptor/bind group layouts, push constants, specialization constants, vertex inputs, render-target formats, storage image usage, and resource access declarations.
  ...
  - [x] Merge vertex/fragment reflection into graphics pipeline layouts.
  - [x] Reflect push constant byte size and stage mask for Slang/SPIR-V paths.
  - [x] Add reflected parameter metadata for resource name, kind, set/binding, count, stage mask, update rate, access, and push-constant size.
  - [x] Validate reflected image and buffer bindings in the frame finalizer, including sampled/storage image usage and uniform/storage buffer usage.
  - [x] Add reflected shader-pass intent API for fullscreen and compute passes with local resource binding and queue inference.
...
- [ ] Add tests for `include_str!` shader compilation, loose-file shader compilation, missing compiler-library packaging diagnostics, shader cache hit/miss behavior, hot reload failure fallback, and reflected binding validation.
  - [x] Add tests for embedded/borrowed source compilation through `ShaderSource::MemoryUtf8`.
  - [x] Add tests for borrowed byte source compilation through `ShaderSource::MemoryBytes`.
  - [x] Add tests for reflected binding metadata and reflected image/buffer validation.
  - [x] Add tests for `engine.load_slang_source` with `SlangEntryPoints::Fragment` and error name propagation.
...
- [ ] Add `ShaderSource` variants:
  - [x] `FilePath`.
  - [x] `VirtualAssetPath` placeholder that requires asset-system resolution before device compilation.
  - [x] `MemoryBytes`.
  - [x] `MemoryUtf8`.
  - [x] Cover `include_str!` through `MemoryUtf8`.
  - [x] Cover `include_bytes!` through `MemoryBytes` for UTF-8 Slang source or SPIR-V bytes.
...
- [ ] For Vulkan, emit SPIR-V and preserve enough reflection metadata to build descriptor layouts, push constants, vertex input declarations, resource states, and graph pass resource declarations.
  - [x] Preserve descriptor layout, push constant, reflected parameter, access, and graph pass binding metadata.
  - [x] Use reflected image/buffer parameters to derive frame graph read/write declarations for shader intents.
  - [ ] Preserve vertex input declarations from reflection.
  - [ ] Preserve specialization constants from reflection.
...
- [ ] Add Vulkan support for reflected bind group updates that can handle generated images, sampled images, samplers, storage images, uniform buffers, storage buffers, and push constants.
  - [x] Add frontend reflected bind group finalization for sampled images, storage images, samplers, uniform buffers, storage buffers, and push constants.
  - [x] Validate reflected image/buffer usage before backend descriptor writes.
  - [ ] Add Vulkan-specific tests covering reflected descriptor updates for each resource kind.
